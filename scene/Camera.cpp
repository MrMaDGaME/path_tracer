...
Camera::Camera(const Vector3 &center, const Vector3 &direction, const Vector3 &up, float beta, float z, int width,
               int height) : center(center), direction(direction.normalize()), up(up.normalize()), beta(beta), z(z),
    ...
    upper_left_corner = center + direction * z + vertical * (viewport_height / 2) -
...
Vector3 Camera::getPixelPosition(int x, int y) const {
    ...
    return upper_left_corner + horizontal * (static_cast<float>(x) * pixel_size) -
