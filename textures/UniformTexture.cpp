...
UniformTexture::UniformTexture(float ks, float kd, float ns, const Color &color) : texture_(
        new Texture(ks, kd, ns, color)) {
}
...
void UniformTexture::reflect(Vector3 point, Vector3 direction, Vector3 normal, Vector3 &lightDirection, Color &filter) {
    ...
    filter = (texture_->color / 255.0f) * (texture_->kd * std::max(0.0f, normal * lightDirection) + texture_->ks *
                                                                                                    std::pow(std::max(
                                                                                                                     0.0f,
                                                                                                                     direction.get_reflection(
                                                                                                                             normal) *
                                                                                                                     lightDirection),
                                                                                                             texture_->ns));
}

Color UniformTexture::getColor(Vector3 point) {
    return texture_->color;
}
